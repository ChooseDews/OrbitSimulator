...
window.setOrbit = function () {
    position = [Number($("#p_x").val()), Number($("#p_y").val()), Number($("#p_z").val())]
    velocity = [Number($("#v_x").val()), Number($("#v_y").val()), Number($("#v_z").val())]
    stats = {
        speed: {
            min: norm(velocity),
            max: norm(velocity),
...
(function () {
    setInterval(function () {
        ...
        updateArrow();
        spacecraft.position.set(position[0] * scaleFactor, position[1] * scaleFactor, position[2] * scaleFactor);
        addPoint(position);
        let v = norm(velocity)
        let r = norm(position)
